...
namespace SistemaInventario.Areas.Inventarios.Controllers
{
    ...
    public class InventarioController : Controller
    {
        ...
        public IActionResult AgregarProductoPost(int producto, int cantidad, int inventarioId) 
        {
            ...
            if(inventarioVM.Inventario.Id == 0) //Grabar nuevo registro
            {
            ...
            var bodegaProducto = _db.BodegaProductos.Include(b => b.Producto).FirstOrDefault(
                b => b.ProductoId == producto && b.BodegaId == inventarioVM.Inventario.BodegaId);

            var detalle = _db.InventarioDetalle.Include(p => p.Producto).FirstOrDefault(
                d => d.ProductoId == producto && d.InventarioId == inventarioVM.Inventario.Id);
            if(detalle == null)
            {
        ...
        public IActionResult GenerarStock(int Id)
        {
            ...
            {
                var bodegaProducto = _db.BodegaProductos.Include(p => p.Producto).FirstOrDefault(b=> b.ProductoId == item.ProductoId && b.BodegaId == inventario.BodegaId);
        ...
        {
            var todos = _db.BodegaProductos.Include(b=>b.Bodega).Include(p=>p.Producto).ToList();
